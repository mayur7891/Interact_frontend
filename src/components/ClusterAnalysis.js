...
const ClusterAnalysis = ({ video_id }) => {
    ...
    const fetchClusterComments = async (clusterId) => {
        try {
            const res = await axios.get(
                `https://flask-app-570571842976.asia-south1.run.app/comments/cluster/${video_id}/${clusterId}`
            );
    ...
    useEffect(() => {
        ...
        axios
            .get(`https://flask-app-570571842976.asia-south1.run.app/comments/unique_clusters/${video_id}`)
            .then(async (res) => {
