...
const Clustered = () => {
    ...
    useEffect(() => {
        if (!video_id) return;
        axios.get(`https://flask-app-993257609003.asia-south1.run.app/comments/unique_clusters/${video_id}`)
            .then((res) => {
    ...
    const fetchClusterComments = async (clusterId) => {
        try {
            const res = await axios.get(`https://flask-app-993257609003.asia-south1.run.app/comments/cluster/${video_id}/${clusterId}`);
    ...
    const handleClustering = () => {
        setLoading(true)
        axios.get(`https://flask-app-993257609003.asia-south1.run.app/ml/get_clusters/${video_id}`)
            .then((res) => {
                if (res.data.message) {
